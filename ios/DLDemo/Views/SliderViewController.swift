...
class SliderViewController: UIViewController, UIScrollViewDelegate {
    ...
    override func viewDidLoad() {
        ...
        if let navigationBarHeight = self.navigationController?.navigationBar.frame.height {
            navigationView.frame = CGRect(x: 0, y: navigationBarHeight, width: view.frame.width, height: 44)
            contentView.frame = CGRect(x: 0, y: 44 + navigationBarHeight, width: view.frame.width, height: contentView.frame.height - navigationBarHeight)
        }
        if !UIApplication.shared.isStatusBarHidden {
            navigationView.frame = CGRect(x: 0, y: navigationView.frame.origin.y + 20, width: view.frame.width, height: 44)
            contentView.frame = CGRect(x: 0, y: contentView.frame.origin.y + 20, width: view.frame.width, height: contentView.frame.height - 20)
        }
...
//        adjustNavigationCellTextColor(highlightIndex: currentIndex)
        if indexPath.row == currentIndex {
            ...
            cell.titleLabel.textColor = DefaultStyleAttribute.cellTitleLabelTextColor
        }
